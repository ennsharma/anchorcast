...
Anchorcast is to generative livestreams what the OpenAI Completions API is to text: one session, a model that turns the next beat into media, a source that decides what that beat is, and a buffer so generation stays ahead of playback.

It is not a YouTube app, not a character, and not a news show. Those belong in apps that sit on top.
...
A `Brief` is the unit of work — the same role `messages` play in Chat Completions. The session asks the source for the next brief, asks the model for a clip, and keeps a small generate-ahead buffer so you do not mint faster than the playhead.
